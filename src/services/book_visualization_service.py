...
class BookVisualization:
    ...
    def plot_genre_counts(self, books: list[Book]) -> None:
        df = pd.DataFrame([{"genre": b.genre} for b in books])
    ...
    def plot_high_rated_genre(self, books: list[Book]) -> None:
        ...
        global_avg = (df["average_rating"] * df["ratings_count"]).sum() / df[
            "ratings_count"
        ].sum()

        m = 50  # min ratings treshold
        genre_stats["bayesian_avg"] = (
            genre_stats["avg_rating"] * genre_stats["ratings_count"] + global_avg * m
        ) / (genre_stats["ratings_count"] + m)

        genre_stats = genre_stats.set_index("genre")
        genre_stats["bayesian_avg"].plot(
            kind="bar", title="Highest Rated Genres (Bayesian Average)"
        )
    ...
    def plot_price_vs_rating(self, books: list[Book]) -> None:
        ...
        plt.scatter(
            df["price_usd"], df["average_rating"], alpha=0.6
        )  # price being x, rating = y, alpha so we can see overlapping points
    ...
    def checkin_vs_available(self, books: list[Book]) -> None:
        df = pd.DataFrame(books)
        
        counts = df["available"].fillna(False).value_counts() #use counts cause plt.pie needs numeric slice sizes
        labels = ["Checked In", "Checked Out"]
        
        plt.pie([counts.get(True, 0), counts.get(False, 0)], labels=labels, autopct="%1.1f%%")#(False, 0) = default to 0 instead of errpr | autopct="%1.1f%%" = display percentages on one decimal place
        plt.axis("equal") #makes the pie chart a perfect cirle (no oval)
